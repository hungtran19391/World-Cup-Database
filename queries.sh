...
echo "$($PSQL "select t.name from teams as t left join games as g on t.team_id = g.winner_id or t.team_id = g.opponent_id where g.year = 2014 and round = 'Eighth-Final' order by t.name asc")"
...
echo "$($PSQL "select g.year, t.name from teams as t left join games as g on t.team_id = g.winner_id where round = 'Final' order by g.year asc")"
